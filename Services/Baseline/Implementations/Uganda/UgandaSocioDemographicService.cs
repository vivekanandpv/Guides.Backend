...
namespace Guides.Backend.Services.Baseline.Implementations.Uganda
{
    public class UgandaSocioDemographicService : IUgandaSocioDemographicService
    {
        ...
        public async Task<SocioDemographicUgandaListViewModel> Register(SocioDemographicUgandaRegisterViewModel viewModel, string initiatedBy)
        {
            ...
            {
                this._logger.LogInformation($"Prevented registration of socio-demographic (Uganda) for ineligible respondent RID: {viewModel.RespondentId}");
                throw new UserActionPreventedException();
            }
            ...
            {
                this._logger.LogInformation($"Prevented registration of socio-demographic (Uganda) for RID: {viewModel.RespondentId} because of registration date discrepancy. Baseline: {respondent.RegisteredOn.Date:dd-MM-yyyy}; Form: {model.RegisteredOn.Date:dd-MM-yyyy}");
