...
{
    public class UgandaDeathRecordService : IUgandaDeathRecordService
    {
        ...
        public UgandaDeathRecordService(
        ...
        public async Task<DeathRecordListViewModel> Get(int id)
        {
            ...
            {
                this._logger.LogInformation($"Death record (Uganda): Cross region access to respondent id: {id} is blocked");
        ...
        public async Task<DeathRecordListViewModel> Register(DeathRecordRegisterViewModel viewModel, string initiatedBy)
        {
            ...
            {
                this._logger.LogInformation($"Prevented registration of death record (Uganda) as user discrepancy found registration data: {viewModel.RegisteredBy}; auth system: {initiatedBy}");
                throw new UserActionNotSupportedException();
            }
            
            this._logger.LogInformation($"death record (Uganda) registration for RID: {viewModel.RespondentId} is initiated");
            ...
            if (respondent == null)
            {
                this._logger.LogInformation($"Prevented registration of death record (Uganda) for non existent RID: {viewModel.RespondentId}");
                throw new UserActionPreventedException();
            }

            if (!respondent.IsEligible)
            {
                this._logger.LogInformation($"Prevented registration of death record (Uganda) for ineligible respondent RID: {viewModel.RespondentId}");
                throw new UserActionPreventedException();
            }
            
            if (respondent.DeathRecord != null)
            {
                this._logger.LogInformation($"Prevented registration of death record (Uganda) for deceased respondent RID: {viewModel.RespondentId}");
                throw new UserActionPreventedException();
            }
            
            if (respondent.LossToFollowUp != null)
            {
                this._logger.LogInformation($"Prevented registration of death record (Uganda) for blocked respondent RID: {viewModel.RespondentId}");
                throw new UserActionPreventedException();
            }

            if (respondent.User.Country != Country.Uganda)
            {
                this._logger.LogInformation($"Prevented cross-region registration of death record (Uganda) for RID: {viewModel.RespondentId}");
                throw new UserActionPreventedException();
            }

            if (respondent.DeathRecord != null)
            {
                this._logger.LogInformation($"Prevented duplicate registration of death record (Uganda) for RID: {viewModel.RespondentId}");
                throw new DuplicatePreventionException();
            }
            
            if (respondent.RegisteredOn.Date > model.RegisteredOn.Date)
            {
                this._logger.LogInformation($"Prevented registration of death record (Uganda) for RID: {viewModel.RespondentId} because of registration date discrepancy. Baseline: {respondent.RegisteredOn.Date:dd-MM-yyyy}; Form: {model.RegisteredOn.Date:dd-MM-yyyy}");
            ...
            this._logger.LogInformation($"Persistence for death record (Uganda) registration for RID: {viewModel.RespondentId} is initiated");
            await this._repository.Add(model);
            this._logger.LogInformation($"Death record (Uganda) registration for RID: {viewModel.RespondentId} succeeded. Id: {respondent.Id}");
        ...
        public async Task<DeathRecordListViewModel> Update(int id, DeathRecordUpdateViewModel viewModel, string initiatedBy)
        {
            ...
            {
                this._logger.LogInformation($"Death record (Uganda): Discrepancy found in id: {id} and data sent: {viewModel.RespondentId}");
                throw new UserActionPreventedException();
            }
            
            var respondentDb = await this._respondentRepository.Get(id);
            
            if (respondentDb?.DeathRecord == null)
            {
                this._logger.LogInformation($"Death record (Uganda): Record not found for RID: {viewModel.RespondentId}");
                throw new RecordNotFoundException();
            }
            
            if (respondentDb.Country != Country.Uganda)
            {
                this._logger.LogInformation($"Death record (Uganda): Cross-origin access for RID: {viewModel.RespondentId}");
                throw new RecordNotFoundException();
            }
            ...
            {
                this._logger.LogInformation($"Death record (Uganda): data update initiated for RID: {viewModel.RespondentId} by {initiatedBy}");
                this._mapper.Map(viewModel, modelDb);
                await this._repository.Save(modelDb);
                
                this._logger.LogInformation($"Death record (Uganda): data update completed for RID: {id} by {initiatedBy}");
