...
{
    public class UgandaPregnancyAndGdmRiskFactorsService : IUgandaPregnancyAndGdmRiskFactorsService
    {
        ...
        public UgandaPregnancyAndGdmRiskFactorsService(
        ...
        public async Task<PregnancyAndGdmRiskFactorsListViewModel> Get(int id)
        {
            ...
            {
                this._logger.LogInformation($"Pregnancy and GDM risk factors (Uganda): Cross region access to respondent id: {id} is blocked");
        ...
        public async Task<PregnancyAndGdmRiskFactorsListViewModel> Register(PregnancyAndGdmRiskFactorsRegisterViewModel viewModel, string initiatedBy)
        {
            ...
            {
                this._logger.LogInformation($"Prevented registration of pregnancy and GDM risk factors (Uganda) as user discrepancy found registration data: {viewModel.RegisteredBy}; auth system: {initiatedBy}");
                throw new UserActionNotSupportedException();
            }
            
            this._logger.LogInformation($"pregnancy and GDM risk factors (Uganda) registration for RID: {viewModel.RespondentId} is initiated");
            ...
            if (respondent == null)
            {
                this._logger.LogInformation($"Prevented registration of pregnancy and GDM risk factors (Uganda) for non existent RID: {viewModel.RespondentId}");
                throw new UserActionPreventedException();
            }

            if (!respondent.IsEligible)
            {
                this._logger.LogInformation($"Prevented registration of pregnancy and GDM risk factors (Uganda) for ineligible respondent RID: {viewModel.RespondentId}");
                throw new UserActionPreventedException();
            }
            
            if (respondent.DeathRecord != null)
            {
                this._logger.LogInformation($"Prevented registration of pregnancy and GDM risk factors (Uganda) for deceased respondent RID: {viewModel.RespondentId}");
                throw new UserActionPreventedException();
            }
            
            if (respondent.LossToFollowUp != null)
            {
                this._logger.LogInformation($"Prevented registration of pregnancy and GDM risk factors (Uganda) for blocked respondent RID: {viewModel.RespondentId}");
                throw new UserActionPreventedException();
            }

            if (respondent.User.Country != Country.Uganda)
            {
                this._logger.LogInformation($"Prevented cross-region registration of pregnancy and GDM risk factors (Uganda) for RID: {viewModel.RespondentId}");
                throw new UserActionPreventedException();
            }

            if (respondent.PregnancyAndGdmRiskFactors != null)
            {
                this._logger.LogInformation($"Prevented duplicate registration of pregnancy and GDM risk factors (Uganda) for RID: {viewModel.RespondentId}");
                throw new DuplicatePreventionException();
            }
            
            if (respondent.RegisteredOn.Date > model.RegisteredOn.Date)
            {
                this._logger.LogInformation($"Prevented registration of pregnancy and GDM risk factors (Uganda) for RID: {viewModel.RespondentId} because of registration date discrepancy. Baseline: {respondent.RegisteredOn.Date:dd-MM-yyyy}; Form: {model.RegisteredOn.Date:dd-MM-yyyy}");
            ...
            this._logger.LogInformation($"Persistence for pregnancy and GDM risk factors (Uganda) registration for RID: {viewModel.RespondentId} is initiated");
            await this._repository.Add(model);
            this._logger.LogInformation($"Pregnancy and GDM risk factors (Uganda) registration for RID: {viewModel.RespondentId} succeeded. Id: {respondent.Id}");
        ...
        public async Task<PregnancyAndGdmRiskFactorsListViewModel> Update(int id, PregnancyAndGdmRiskFactorsUpdateViewModel viewModel, string initiatedBy)
        {
            ...
            {
                this._logger.LogInformation($"Pregnancy and GDM risk factors (Uganda): Discrepancy found in id: {id} and data sent: {viewModel.RespondentId}");
                throw new UserActionPreventedException();
            }
            ...
            {
                this._logger.LogInformation($"Pregnancy and GDM risk factors (Uganda): Record not found for RID: {viewModel.RespondentId}");
                throw new RecordNotFoundException();
            }
            
            if (respondentDb.Country != Country.Uganda)
            {
                this._logger.LogInformation($"Pregnancy and GDM risk factors (Uganda): Cross-origin access for RID: {viewModel.RespondentId}");
                throw new RecordNotFoundException();
            }
            ...
            {
                this._logger.LogInformation($"Pregnancy and GDM risk factors (Uganda): data update initiated for RID: {viewModel.RespondentId} by {initiatedBy}");
                this._mapper.Map(viewModel, modelDb);
                await this._repository.Save(modelDb);
                
                this._logger.LogInformation($"Pregnancy and GDM risk factors (Uganda): data update completed for RID: {id} by {initiatedBy}");
