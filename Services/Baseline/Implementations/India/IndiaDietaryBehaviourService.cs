...
namespace Guides.Backend.Services.Baseline.Implementations.India
{
    public class IndiaDietaryBehaviourService : IIndiaDietaryBehaviourService
    {
        ...
        public async Task<DietaryBehaviourListViewModel> Register(DietaryBehaviourRegisterViewModel viewModel, string initiatedBy)
        {
            ...
            {
                this._logger.LogInformation($"Prevented registration of dietary behaviour (India) for ineligible respondent RID: {viewModel.RespondentId}");
                throw new UserActionPreventedException();
            }
            ...
            {
                this._logger.LogInformation($"Prevented registration of dietary behaviour (India) for RID: {viewModel.RespondentId} because of registration date discrepancy. Baseline: {respondent.RegisteredOn.Date:dd-MM-yyyy}; Form: {model.RegisteredOn.Date:dd-MM-yyyy}");
