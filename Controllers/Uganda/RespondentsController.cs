...
namespace Guides.Backend.Controllers.Uganda
{
    ...
    [Route(EndpointStaticStore.RespondentsUgandaTemplate)]
    [ApiController]
    public class RespondentsController : ControllerBase
    {
        ...
        public RespondentsController(IUgandaRespondentService service, IAppUtils appUtils)
        {
            _service = service;
            _appUtils = appUtils;
        }
        ...
        [HttpGet]
        public async Task<ActionResult<IEnumerable<RespondentUgandaListViewModel>>> Get() => Ok(await this._service.Get());

        [HttpGet(EndpointStaticStore.GetById)]
        public async Task<ActionResult<RespondentUgandaListViewModel>> Get(int id) => Ok(await this._service.Get(id));

        [HttpPost]
        public async Task<ActionResult<RespondentUgandaListViewModel>> Register(
            RespondentUgandaRegisterViewModel viewModel)
            => Ok(await this._service.Register(viewModel, this._appUtils.GetCurrentUser(HttpContext)));

        [HttpPut(EndpointStaticStore.GetById)]
        public async Task<ActionResult<RespondentUgandaListViewModel>> Update(
            int id, RespondentUgandaUpdateViewModel viewModel)
